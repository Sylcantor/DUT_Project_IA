...
from games.TicTacToe.transform import Transform, Identity, Rotate90, Flip
from api.prepare import is_even
...
class TicTacToe:
    ...
    def play_move(self, move_index):
        ...
        if move_index not in self.get_valid_move_indexes():
            return TicTacToe(board_copy, illegal_move=move_index)

        board_copy[move_index] = self.get_turn()
        return TicTacToe(board_copy)
