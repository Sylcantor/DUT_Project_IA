...
class Nim:
    ...
    def main():
        ...
        q_vs_opt_train = Nim(qAgent, optAgent, 100)  # 100000

        res = random.sample(range(1, 10), 3)

        q_vs_opt_train.play(State(res))
        print("Train - Wins: ", q_vs_opt_train.wins,
              "Losses: ", q_vs_opt_train.losses)

        qAgent.epsilon = 0
        q_vs_opt_test = Nim(qAgent, optAgent, 100)

        res = random.sample(range(1, 10), 3)

        q_vs_opt_test.play(State(res))
        print("Test - Wins: ", q_vs_opt_test.wins,
              "Losses: ", q_vs_opt_test.losses)
